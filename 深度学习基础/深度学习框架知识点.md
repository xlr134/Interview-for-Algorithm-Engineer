...
### view方法的限制
view方法能够将tensor转换为指q定的shape，且原始的data不改变。返回的tensor与原始的tensor共享存储区。但view方法需要满足以下连续条件：
...
### expand()
- $\textcolor{red}{不会分配新内存，只是创建一个新的视图}$
- 只能扩展大小为1的维度
...
### flatten()
将张量展平成一维($\textcolor{red}{NumPy 的 `flatten()` 总是返回副本，而 PyTorch 的 `flatten()` 在输入张量连续时返回视图，不连续时返回副本。}$)

$\textcolor{red}{(flatten默认start_dim=1,end_dim=-1)}$
...
### ravel()
功能类似于flatten()，但返回的可能是一个视图（$\textcolor{red}{返回一个连续展平的张量。仅在需要时才会创建副本。}$）
...
### split()
$\textcolor{red}{将张量拆分成多个块。每个块是原始张量的一个视图}$

torch.split(*tensor*, *split_size_or_sections*, *dim=0*)

![image-20250416143714422](imgs\image-20250416143714422.png)
...
![image-20250416144313765](imgs\image-20250416144313765.png)
...
![image-20250416150016256](imgs\image-20250416150016256.png)
...
model.eval()
#apply(fn) 递归地将 fn 应用到每个子模块（由 .children() 返回）以及自身。
```
...
#该容器通过在批次维度上对输入进行切块并在指定设备之间分配，来并行化给定 module 的应用。在前向传播过程中，模块会在每个设备上进行复制，每个副本处理输入的一个部#分。在反向传播过程中，来自每个副本的梯度会被加总到原始模块中。
```
...
TorchScript基本上是一种以更静态和优化的形式表示PyTorch模型计算的方法。与python运行时解耦。

1. **优化**：通过拥有[计算图](https://zhida.zhihu.com/search?content_id=255267970&content_type=Article&match_order=1&q=计算图&zhida_source=entity)的静态表示，TorchScript允许运行时执行各种优化，这些优化在Python前端的动态特性下是不可能的。这可以提高性能。
2. **可移植性**：TorchScript模型可以独立于创建它们的原始Python代码进行保存和加载。这使得共享模型或在非Python编写的应用程序中使用它们变得容易。
3. **集成**：有各种工具和语言可以与TorchScript接口，但可能无法很好地与纯Python模型一起使用。例如，您可以将TorchScript模型集成到C++应用程序、移动应用程序或WebAssembly（需要一些额外的工作）中。

（$\textcolor{red}{静态图用户的模型会先被转换为静态图，静态图知道整个计算过程，系统可以预测各个节点的计算时间，计算路径是确定的。}$）
